...
if __name__ == '__main__':
    print [str(m.text) for m in messages]
    print [str(m.text) for m in sorted(messages,key=lambda x: x.date)][::-1]
    print [str(m.text) for m in sorted(messages, key=lambda x: x.author)]